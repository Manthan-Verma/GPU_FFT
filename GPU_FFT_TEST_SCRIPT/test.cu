...
int main(int argc, char *argv[])
{
    ...
    TRANSITIONS::set_gpu_device(rank);
    ...
    data_gpu_in = TRANSITIONS::Memory_allocation_gpu(data_gpu_in, (Nx / procs) * Ny * (Nz / 2 + 1));
    ...
    TRANSITIONS::Memory_copy_cpu_to_gpu(data_cpu_in, data_gpu_in, (Nx / procs) * Ny * (Nz / 2 + 1));
    ...
    TRANSITIONS::Memory_copy_gpu_to_cpu(data_gpu_in, data_cpu_out, (Nx / procs) * Ny * (Nz / 2 + 1));
